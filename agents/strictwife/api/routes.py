"""
Strictwife AI REST API Routes
Production-ready HTTP endpoints for strictwife agent
"""
...
from ..services.cortex.controller import StrictwifeController
from ..monitor.usage import track_usage
from ..analytics.metrics import StrictwifeMetrics
from ..analytics.logger import StrictwifeAnalyticsLogger

# Initialize blueprint and services
strictwife_api = Blueprint("strictwife_api", __name__, url_prefix="/api/strictwife")
controller = StrictwifeController()
metrics = StrictwifeMetrics()
analytics_logger = StrictwifeAnalyticsLogger()
...
@strictwife_api.route("/health", methods=["GET"])
def health_check():
...
@strictwife_api.route("/chat", methods=["POST"])
@rate_limit
...
@strictwife_api.route("/analytics", methods=["GET"])
def get_analytics():
...
@strictwife_api.route("/speak", methods=["POST"])
@rate_limit
@validate_request(["text"])
def speak_text():
    """Convert text to speech with strict, disciplinary voice"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
    ...
    try:
        ...
        # Generate speech with strict personality
        ...
        response = {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "request_id": request_id,
                    "timestamp": time.time(),
                    "voice_personality": "stern_disciplinary"
                }
            }
        }
...
@strictwife_api.route("/voice/capabilities", methods=["GET"])
    ...
    try:
        ...
        response = {
            ...
            "data": {
                **voice_caps,
                "agent": "strictwife",
                "voice_description": "Stern, authoritative voice with disciplinary tone",
                "personality_traits": [
                    "authoritative",
                    "disciplinary", 
                    "firm_guidance",
...
@strictwife_api.route("/chat/speak", methods=["POST"])
@rate_limit
@validate_request(["message"])
def chat_with_voice():
    """Chat with strict voice response"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
