...
Production-ready HTTP endpoints for content_creator agent
"""
...
from ..services.cortex.controller import ContentcreatorController
from ..monitor.usage import track_usage
from ..analytics.metrics import ContentcreatorMetrics
from ..analytics.logger import ContentcreatorAnalyticsLogger

# Initialize blueprint and services
content_creator_api = Blueprint(
    "content_creator_api", __name__, url_prefix="/api/content_creator"
)
controller = ContentcreatorController()
metrics = ContentcreatorMetrics()
analytics_logger = ContentcreatorAnalyticsLogger()
...
@content_creator_api.route("/health", methods=["GET"])
def health_check():
...
@content_creator_api.route("/chat", methods=["POST"])
@rate_limit
...
@content_creator_api.route("/analytics", methods=["GET"])
def get_analytics():
...
@content_creator_api.route("/speak", methods=["POST"])
@rate_limit
@validate_request(["text"])
def speak_text():
    """Convert text to speech with creative, engaging storytelling voice"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
    ...
    try:
        ...
        # Generate speech with content creator personality
        ...
        response = {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "request_id": request_id,
                    "timestamp": time.time(),
                    "voice_personality": "creative_storytelling"
                }
            }
        }
...
@content_creator_api.route("/voice/capabilities", methods=["GET"])
    ...
    try:
        ...
        response = {
            ...
            "data": {
                **voice_caps,
                "agent": "content_creator",
                "voice_description": "Creative, engaging voice with storytelling narrative tone",
                "personality_traits": [
                    "creative",
                    "engaging", 
                    "storytelling",
...
@content_creator_api.route("/chat/speak", methods=["POST"])
@rate_limit
@validate_request(["message"])
def chat_with_voice():
    """Chat with creative storytelling voice response"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
