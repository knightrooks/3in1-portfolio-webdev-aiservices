...
Production-ready HTTP endpoints for marketing_specialist agent
"""
...
from ..services.cortex.controller import MarketingspecialistController
from ..monitor.usage import track_usage
from ..analytics.metrics import MarketingspecialistMetrics
from ..analytics.logger import MarketingspecialistAnalyticsLogger

# Initialize blueprint and services
marketing_specialist_api = Blueprint(
    "marketing_specialist_api", __name__, url_prefix="/api/marketing_specialist"
)
controller = MarketingspecialistController()
metrics = MarketingspecialistMetrics()
analytics_logger = MarketingspecialistAnalyticsLogger()
...
@marketing_specialist_api.route("/health", methods=["GET"])
def health_check():
...
@marketing_specialist_api.route("/chat", methods=["POST"])
@rate_limit
...
@marketing_specialist_api.route("/analytics", methods=["GET"])
def get_analytics():
...
@marketing_specialist_api.route("/speak", methods=["POST"])
@rate_limit
@validate_request(["text"])
def speak_text():
    """Convert text to speech with persuasive, engaging marketing voice"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
    ...
    try:
        ...
        # Generate speech with marketing specialist personality
        ...
        response = {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "request_id": request_id,
                    "timestamp": time.time(),
                    "voice_personality": "persuasive_engaging"
                }
            }
        }
...
@marketing_specialist_api.route("/voice/capabilities", methods=["GET"])
    ...
    try:
        ...
        response = {
            ...
            "data": {
                **voice_caps,
                "agent": "marketing_specialist",
                "voice_description": "Persuasive, engaging voice with dynamic marketing tone",
                "personality_traits": [
                    "persuasive",
                    "engaging", 
                    "dynamic",
...
@marketing_specialist_api.route("/chat/speak", methods=["POST"])
@rate_limit
@validate_request(["message"])
def chat_with_voice():
    """Chat with persuasive marketing voice response"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
