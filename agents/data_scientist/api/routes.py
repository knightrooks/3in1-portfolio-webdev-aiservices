...
Production-ready HTTP endpoints for data_scientist agent
"""
...
from ..services.cortex.controller import DatascientistController
from ..monitor.usage import track_usage
from ..analytics.metrics import DatascientistMetrics
from ..analytics.logger import DatascientistAnalyticsLogger

# Initialize blueprint and services
data_scientist_api = Blueprint(
    "data_scientist_api", __name__, url_prefix="/api/data_scientist"
)
controller = DatascientistController()
metrics = DatascientistMetrics()
analytics_logger = DatascientistAnalyticsLogger()
...
@data_scientist_api.route("/health", methods=["GET"])
def health_check():
...
@data_scientist_api.route("/chat", methods=["POST"])
@rate_limit
...
@data_scientist_api.route("/analytics", methods=["GET"])
def get_analytics():
...
@data_scientist_api.route("/speak", methods=["POST"])
@rate_limit
@validate_request(["text"])
def speak_text():
    """Convert text to speech with analytical, data-driven voice"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
    ...
    try:
        ...
        # Generate speech with data scientist personality
        ...
        response = {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "request_id": request_id,
                    "timestamp": time.time(),
                    "voice_personality": "analytical_data_driven"
                }
            }
        }
...
@data_scientist_api.route("/voice/capabilities", methods=["GET"])
    ...
    try:
        ...
        response = {
            ...
            "data": {
                **voice_caps,
                "agent": "data_scientist",
                "voice_description": "Analytical, data-driven voice with scientific precision tone",
                "personality_traits": [
                    "analytical",
                    "data_driven", 
                    "scientific",
...
@data_scientist_api.route("/chat/speak", methods=["POST"])
@rate_limit
@validate_request(["message"])
def chat_with_voice():
    """Chat with analytical data scientist voice response"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
