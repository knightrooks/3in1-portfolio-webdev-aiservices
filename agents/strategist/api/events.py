...
class StrategistEventManager:
    ...
    def get_events(self, session_id: Optional[str] = None, 
                   ...
                   limit: int = 100) -> List[StrategistEvent]:
    ...
    def get_event_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'events_per_hour': len(events) / (time_range / 3600) if time_range > 0 else 0,
            'most_common_event': max(event_counts.items(), key=lambda x: x[1])[0] if event_counts else None,
            'recent_activity': len([e for e in events if time.time() - e.timestamp < 3600])  # Last hour
        }
...
def emit_consultation_started(session_id: str, consultation_type: str, client_info: Dict[str, Any]):
...
def emit_consultation_completed(session_id: str, duration: float, deliverables: List[str], 
...
def emit_strategy_generated(session_id: str, strategy_type: str, components: List[str], 
