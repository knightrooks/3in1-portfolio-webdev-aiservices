"""
Lazyjohn AI REST API Routes
Production-ready HTTP endpoints for lazyjohn agent
"""
...
lazyjohn_api = Blueprint("lazyjohn_api", __name__, url_prefix="/api/lazyjohn")
...
@lazyjohn_api.route("/health", methods=["GET"])
def health_check():
...
@lazyjohn_api.route("/chat", methods=["POST"])
@rate_limit
...
@lazyjohn_api.route("/analytics", methods=["GET"])
def get_analytics():
...
@lazyjohn_api.route("/speak", methods=["POST"])
@rate_limit
@validate_request(["text"])
def speak_text():
    """Convert text to speech with lazy, monotone casual voice"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
    ...
    try:
        ...
        # Generate speech with lazy personality
        ...
        response = {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "request_id": request_id,
                    "timestamp": time.time(),
                    "voice_personality": "monotone_casual"
                }
            }
        }
...
@lazyjohn_api.route("/voice/capabilities", methods=["GET"])
    ...
    try:
        ...
        response = {
            ...
            "data": {
                **voice_caps,
                "agent": "lazyjohn",
                "voice_description": "Lazy, monotone voice with casual laid-back tone",
                "personality_traits": [
                    "monotone",
                    "casual", 
                    "laid_back",
...
@lazyjohn_api.route("/chat/speak", methods=["POST"])
@rate_limit
@validate_request(["message"])
def chat_with_voice():
    """Chat with lazy, casual voice response"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
