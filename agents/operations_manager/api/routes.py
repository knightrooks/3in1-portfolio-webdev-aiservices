...
Production-ready HTTP endpoints for operations_manager agent
"""
...
from ..services.cortex.controller import OperationsmanagerController
from ..monitor.usage import track_usage
from ..analytics.metrics import OperationsmanagerMetrics
from ..analytics.logger import OperationsmanagerAnalyticsLogger

# Initialize blueprint and services
operations_manager_api = Blueprint(
    "operations_manager_api", __name__, url_prefix="/api/operations_manager"
)
controller = OperationsmanagerController()
metrics = OperationsmanagerMetrics()
analytics_logger = OperationsmanagerAnalyticsLogger()
...
@operations_manager_api.route("/health", methods=["GET"])
def health_check():
...
@operations_manager_api.route("/chat", methods=["POST"])
@rate_limit
...
@operations_manager_api.route("/analytics", methods=["GET"])
def get_analytics():
...
@operations_manager_api.route("/speak", methods=["POST"])
@rate_limit
@validate_request(["text"])
def speak_text():
    """Convert text to speech with efficient, organized operations voice"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
    ...
    try:
        ...
        # Generate speech with operations manager personality
        ...
        response = {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "request_id": request_id,
                    "timestamp": time.time(),
                    "voice_personality": "efficient_organized"
                }
            }
        }
...
@operations_manager_api.route("/voice/capabilities", methods=["GET"])
    ...
    try:
        ...
        response = {
            ...
            "data": {
                **voice_caps,
                "agent": "operations_manager",
                "voice_description": "Efficient, organized voice with systematic operational tone",
                "personality_traits": [
                    "efficient",
                    "organized", 
                    "systematic",
...
@operations_manager_api.route("/chat/speak", methods=["POST"])
@rate_limit
@validate_request(["message"])
def chat_with_voice():
    """Chat with efficient operations manager voice response"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
