"""
Girlfriend AI REST API Routes
Production-ready HTTP endpoints for girlfriend agent
"""
...
from ..services.cortex.controller import GirlfriendController
...
girlfriend_api = Blueprint("girlfriend_api", __name__, url_prefix="/api/girlfriend")
controller = GirlfriendController()
metrics = GirlfriendMetrics()
analytics_logger = GirlfriendAnalyticsLogger()
...
@girlfriend_api.route("/health", methods=["GET"])
def health_check():
...
@girlfriend_api.route("/chat", methods=["POST"])
@rate_limit
...
@girlfriend_api.route("/analytics", methods=["GET"])
def get_analytics():
...
@girlfriend_api.route("/speak", methods=["POST"])
@rate_limit
@validate_request(["text"])
def speak_text():
    """Convert text to speech with sweet, affectionate girlfriend voice"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
    ...
    try:
        ...
        # Generate speech with girlfriend personality
        ...
        response = {
            ...
            "data": {
                ...
                "metadata": {
                    ...
                    "request_id": request_id,
                    "timestamp": time.time(),
                    "voice_personality": "sweet_affectionate"
                }
            }
        }
...
@girlfriend_api.route("/voice/capabilities", methods=["GET"])
    ...
    try:
        ...
        response = {
            ...
            "data": {
                **voice_caps,
                "agent": "girlfriend",
                "voice_description": "Sweet, affectionate voice with loving romantic tone",
                "personality_traits": [
                    "sweet",
                    "affectionate", 
                    "romantic",
...
@girlfriend_api.route("/chat/speak", methods=["POST"])
@rate_limit
@validate_request(["message"])
def chat_with_voice():
    """Chat with sweet, affectionate girlfriend voice response"""
    start_time = time.time()
    request_id = str(uuid.uuid4())
