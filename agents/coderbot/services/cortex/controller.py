"""
Coder Bot Controller
Main processing logic for coderbot agent
"""

import logging
from typing import Dict, Any, List
from .planner import CoderbotPlanner  
from .executor import CoderbotExecutor
from ..brain.episodic import EpisodicMemory
from ..engine.dispatcher import ModelDispatcher

logger = logging.getLogger(__name__)

class CoderbotController:
    """Main controller for Coder Bot agent"""
    
    def __init__(self):
        self.planner = CoderbotPlanner()
        self.executor = CoderbotExecutor()
        self.memory = EpisodicMemory(agent_id='coderbot')
        self.dispatcher = ModelDispatcher(['deepseek_coder', 'qwen2_5_coder'])
        
        # Personality configuration
        self.personality_traits = ['logical', 'precise', 'technical', 'systematic', 'problem_solving', 'efficiency_focused']
        self.response_style = {'tone': 'technical_precise', 'formality': 'professional', 'accuracy': 'high', 'code_focus': 'maximum'}
    ...
    def _get_fallback_response(self) -> str:
        ...
        return fallback_responses.get('Technical', "I didn't understand that. Could you please rephrase?")
