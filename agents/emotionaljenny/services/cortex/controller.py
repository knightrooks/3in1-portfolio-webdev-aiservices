"""
Emotional Jenny Controller
Main processing logic for emotionaljenny agent
"""

import logging
from typing import Dict, Any, List
from .planner import EmotionaljennyPlanner  
from .executor import EmotionaljennyExecutor
from ..brain.episodic import EpisodicMemory
from ..engine.dispatcher import ModelDispatcher

logger = logging.getLogger(__name__)

class EmotionaljennyController:
    """Main controller for Emotional Jenny agent"""
    
    def __init__(self):
        self.planner = EmotionaljennyPlanner()
        self.executor = EmotionaljennyExecutor()
        self.memory = EpisodicMemory(agent_id='emotionaljenny')
        self.dispatcher = ModelDispatcher(['llama3_2'])
        
        # Personality configuration
        self.personality_traits = ['highly_emotional', 'empathetic', 'sensitive', 'intuitive', 'compassionate', 'nurturing']
        self.response_style = {'tone': 'deeply_caring', 'formality': 'gentle_formal', 'empathy_level': 'maximum', 'emotional_depth': 'very_deep'}
    ...
    def _get_fallback_response(self) -> str:
        ...
        return fallback_responses.get('Emotional', "I didn't understand that. Could you please rephrase?")
