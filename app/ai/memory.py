...
class MemoryManager:
    ...
    async def search_memory(
        self, query: str, memory_types: List[str] = None, max_results: int = 10
    ) -> List[Dict]:
        ...
        if "knowledge" in memory_types:
            knowledge_results = await self.get_relevant_knowledge(
                query, max_results // 3
            )
            results.extend(
                [{"type": "knowledge", "data": k} for k in knowledge_results]
            )
    ...
    async def _update_context_cache(self, session_id: str, interaction: Dict):
        ...
        await self.update_context(
            session_id, "last_activity", datetime.now().isoformat()
        )
    ...
    async def _extract_knowledge(self, interaction: Dict):
        ...
        if any(
            ...
            for indicator in ["what is", "how to", "explain", "define"]
        ):
    ...
    def get_usage_stats(self) -> Dict:
        ...
        return {
            ...
            "short_term_interactions": sum(
                len(interactions) for interactions in self.short_term_memory.values()
            ),
