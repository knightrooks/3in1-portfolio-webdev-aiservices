...
class TaskPlanner:
    ...
    async def create_plan(
        self, message: str, agent_type: str, context: Dict = None, history: List = None
    ) -> Dict:
    ...
    def _classify_intent(self, message: str) -> str:
        ...
        if any(
            word in message_lower for word in ["strategy", "plan", "business", "market"]
        ):
            return "strategic_planning"
        elif any(
            word in message_lower for word in ["code", "develop", "build", "implement"]
        ):
            return "development"
        elif any(
            word in message_lower for word in ["write", "content", "article", "copy"]
        ):
            return "content_creation"
        elif any(
            word in message_lower
            for word in ["security", "vulnerability", "threat", "protect"]
        ):
            return "security_analysis"
        elif any(
            word in message_lower for word in ["analyze", "research", "investigate"]
        ):
            return "analysis"
        else:
    ...
    def _analyze_history_patterns(self, history: List) -> Dict:
        ...
        return {
            ...
            "avg_length": sum(len(msg.get("content", "")) for msg in recent_messages)
            / len(recent_messages),
        }
    ...
    async def _decompose_tasks(
        self, analysis: Dict, agent_config: Dict, complexity: str
    ) -> List[Dict]:
        """Decompose the main task into subtasks."""
        base_steps = agent_config.get(
            "default_steps", ["analyze", "process", "respond"]
        )
