...
class TaskExecutor:
    ...
    async def execute_plan(self, plan: Dict, models: Dict, session_context: Dict = None) -> Dict:
        ...
        try:
            ...
            results = await self._execute_tasks_with_dependencies(tasks, models, execution)
    ...
    async def _execute_tasks_with_dependencies(self, tasks: List[Dict], models: Dict, execution: Dict) -> Dict:
        ...
        while len(completed_tasks) < len(tasks):
            ...
            if not ready_tasks:
                ...
                raise Exception(f"Circular dependency detected in tasks: {[t['id'] for t in remaining_tasks]}")
            
            # Execute ready tasks (can be done in parallel)
            batch_results = await self._execute_task_batch(ready_tasks, models, results, execution)
    ...
    async def _execute_task_batch(self, tasks: List[Dict], models: Dict, previous_results: Dict, execution: Dict) -> Dict:
    ...
    async def _execute_single_task(self, task: Dict, models: Dict, previous_results: Dict, execution: Dict) -> Dict:
    ...
    def _select_model_for_task(self, task: Dict, models: Dict) -> str:
        ...
        # Code-related tasks
        if any(keyword in task_name + task_desc for keyword in ['code', 'develop', 'implement']):
            for model in ['deepseek-coder', 'codellama']:
                if model in available_models:
                    return model
        
        # Security tasks
        if any(keyword in task_name + task_desc for keyword in ['security', 'vulnerability', 'threat']):
            for model in ['llama3.2', 'deepseek-coder']:
                if model in available_models:
                    return model
        
        # Strategic/analytical tasks
        if any(keyword in task_name + task_desc for keyword in ['strategy', 'analyze', 'plan']):
    ...
    async def _execute_processing_task(self, task: Dict, model: Any, context: Dict) -> Dict:
    ...
    async def _execute_verification_task(self, task: Dict, model: Any, context: Dict) -> Dict:
    ...
    async def _execute_generic_task(self, task: Dict, model: Any, context: Dict) -> Dict:
...
"""
    ...
    async def _compile_response(self, results: Dict, plan: Dict, execution: Dict) -> Dict:
        """Compile final response from task results."""
        # Aggregate all task results
        successful_tasks = [
            task_result for task_result in results.values() 
        ...
        if failed_tasks:
            # Partial completion
            response_content = self._create_partial_response(successful_tasks, failed_tasks, plan)
            status = 'partial'
        else:
            # Full completion
        ...
        return {
            ...
            'metadata': {
                ...
                'models_used': list(set([
                    task.get('model_used', 'unknown') 
                    for task in successful_tasks
                ]))
            }
        }
    
    def _create_complete_response(self, successful_tasks: List[Dict], plan: Dict) -> str:
        """Create response for successfully completed plan."""
        # Extract key results from tasks
        key_outputs = []
        for task in successful_tasks:
            result = task.get('result', {})
            output = result.get('output') or result.get('verification_result') or "Task completed"
    ...
    def _create_partial_response(self, successful_tasks: List[Dict], failed_tasks: List[Dict], plan: Dict) -> str:
        """Create response for partially completed plan."""
        response = "I've completed most of your request, though I encountered some issues:\n\n"
