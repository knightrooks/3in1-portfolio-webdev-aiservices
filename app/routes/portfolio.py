...
def projects():
    ...
    if category == "all":
        ...
        projects = [
            p for p in PORTFOLIO_DATA["featured_projects"] if p["category"] == category
        ]
...
def project_detail(project_id):
    ...
    project = next(
        (p for p in PORTFOLIO_DATA["featured_projects"] if p["id"] == project_id), None
    )
    ...
    related_projects = [
        p
        for p in PORTFOLIO_DATA["featured_projects"]
...
def analytics():
    ...
    analytics_data = {
        ...
        "skill_stats": {
            ...
            "average_skill_level": sum(s["level"] for s in PORTFOLIO_DATA["skills"])
            / len(PORTFOLIO_DATA["skills"]),
            "skill_categories": {},
        },
        "specialty_stats": {
            "total_specialties": len(PORTFOLIO_DATA["specialties"]),
            "total_experience_years": sum(
                s["experience_years"] for s in PORTFOLIO_DATA["specialties"]
            ),
            "average_experience": sum(
                s["experience_years"] for s in PORTFOLIO_DATA["specialties"]
            )
            / len(PORTFOLIO_DATA["specialties"]),
        },
        "achievement_stats": {
            "total_achievements": len(PORTFOLIO_DATA["achievements"]),
            "recent_achievements": len(
                [a for a in PORTFOLIO_DATA["achievements"] if "2025" in a["date"]]
            ),
        },
    }
    ...
    for project in PORTFOLIO_DATA["featured_projects"]:
        ...
        analytics_data["project_stats"]["categories"][category] = (
            analytics_data["project_stats"]["categories"].get(category, 0) + 1
        )

    # Skill categories
    for skill in PORTFOLIO_DATA["skills"]:
        category = skill["category"]
        analytics_data["skill_stats"]["skill_categories"][category] = (
            analytics_data["skill_stats"]["skill_categories"].get(category, 0) + 1
        )
