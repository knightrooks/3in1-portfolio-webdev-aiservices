...
MODEL_TYPES = {
    ...
    'deepseek-coder': {
        ...
        'category': 'code',
        'use_cases': ['code_completion', 'bug_fixing', 'refactoring', 'code_explanation'],
    ...
    'mathstral': {
        ...
        'category': 'math',
        'use_cases': ['mathematical_analysis', 'calculations', 'data_modeling', 'statistics'],
    ...
    'mistral': {
        ...
        'category': 'general',
        'use_cases': ['conversation', 'text_generation', 'summarization', 'translation'],
    ...
    'nomic-embed-text': {
        ...
        'category': 'embedding',
        'use_cases': ['semantic_search', 'text_similarity', 'document_analysis', 'clustering'],
    ...
    'phi3': {
        ...
        'category': 'reasoning',
        'use_cases': ['logical_reasoning', 'problem_solving', 'decision_making', 'analysis'],
    ...
    'qwen2.5': {
        ...
        'use_cases': ['multilingual_conversation', 'translation', 'cultural_analysis', 'global_support'],
    ...
    'qwen2.5-coder': {
        ...
        'category': 'code',
        'use_cases': ['code_generation', 'code_review', 'algorithm_design', 'optimization'],
    ...
    'snowflake-arctic-embed': {
        ...
        'category': 'embedding',
        'use_cases': ['enterprise_search', 'document_retrieval', 'knowledge_base', 'analytics'],
    ...
    'yi': {
        ...
        'category': 'multilingual',
        'use_cases': ['bilingual_support', 'cross_cultural_communication', 'translation', 'localization'],
...
def models_api_overview():
    ...
    return jsonify({
        ...
        'data': {
            'total_models': len(MODEL_TYPES),
            'categories': list(set(model['category'] for model in MODEL_TYPES.values())),
            'models': model_apis,
            'system_resources': get_system_resources()
        }
    })
...
def model_detail(model_id):
    ...
    for agent_id in model['agents']:
        try:
            ...
            if hasattr(agent_module, 'ALL_AGENTS') and agent_id in agent_module.ALL_AGENTS:
                connected_agents.append({
                    'id': agent_id,
                    'info': agent_module.ALL_AGENTS[agent_id]
                })
        except ImportError:
            pass
    
    return render_template('models/detail.html',
                         model_id=model_id,
                         model=model,
                         status=status,
...
def start_model(model_id):
    ...
    try:
        ...
        result = subprocess.run(['ollama', 'run', model_id], 
...
def model_health(model_id):
    ...
    health_data['overall_health'] = 'healthy' if health_data['health_score'] > 75 else 'degraded' if health_data['health_score'] > 50 else 'unhealthy'
...
def models_analytics():
    ...
    for model_id, model_info in MODEL_TYPES.items():
        for agent in model_info['agents']:
            analytics_data['agent_connections'][agent] = analytics_data['agent_connections'].get(agent, 0) + 1
...
def models_health():
    ...
    return jsonify({
        'success': True,
        'data': {
            'overall_status': 'healthy' if healthy_count > 0 and system_healthy else 'degraded',
