...
class NotificationService:
    ...
    def _configure_smtp(self):
        ...
        try:
            ...
            self.from_name = current_app.config.get('FROM_NAME', 'AI Portfolio Platform')
    ...
    def start_notification_worker(self):
        ...
        if not self.is_running:
            ...
            self.notification_worker = threading.Thread(target=self._notification_worker_thread)
    ...
    def send_email(self, to_email, subject, template_name=None, template_data=None, 
                   html_content=None, text_content=None, attachments=None, async_send=True):
    ...
    def _send_email_sync(self, to_email, subject, template_name=None, template_data=None,
                         html_content=None, text_content=None, attachments=None):
        ...
        try:
            ...
            if template_name:
                html_content, text_content = self._render_email_template(template_name, template_data or {})
    ...
    def send_push_notification(self, user_tokens, title, body, data=None):
        ...
        try:
            ...
            payload = {
                'registration_ids': user_tokens if isinstance(user_tokens, list) else [user_tokens],
                'notification': {
                    'title': title,
                    'body': body,
            ...
            if response.status_code == 200:
                logger.info(f"Push notification sent to {len(user_tokens) if isinstance(user_tokens, list) else 1} devices")
                return True
            else:
    ...
    def send_in_app_notification(self, user_id, title, message, notification_type='info', action_url=None):
    ...
    def notify_payment_success(self, user, payment):
        ...
        try:
            template_data = {
                ...
                'payment_date': payment.completed_at.strftime('%B %d, %Y') if payment.completed_at else 'Today',
                'dashboard_url': f"{current_app.config.get('BASE_URL', 'http://localhost:5000')}/dashboard"
            }
