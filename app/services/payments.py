...
class PaymentProcessor:
            ...
            plans.append(
                {
                    ...
                    "is_premium": self.agent_pricing.get(agent_id, {}).get(
                        "premium", False
                    ),
                }
            )
        ...
        try:
            ...
            if event["type"] == "payment_intent.succeeded":
                ...
                return self._handle_successful_payment(
                    payment_intent["metadata"], "stripe", payment_intent["id"]
                )
    ...
    def process_paypal_webhook(self, data: Dict) -> Dict:
        ...
        try:
            ...
            if event_type == "PAYMENT.SALE.COMPLETED":
                ...
                return self._handle_successful_payment(
                    custom_data, "paypal", sale["id"]
                )

            elif event_type == "PAYMENT.SALE.DENIED":
                sale = data["resource"]
                custom_data = json.loads(sale.get("custom", "{}"))
                return self._handle_failed_payment(custom_data, "paypal", sale["id"])
    ...
    def _handle_successful_payment(
        self, metadata: Dict, payment_method: str, transaction_id: str
    ) -> Dict:
    ...
    def _handle_failed_payment(
        ...
        try:
            ...
            logger.warning(
                f"Payment failed: {user_id} -> {agent_id} ({payment_method})"
            )
    ...
    def check_subscription_access(self, user_id: str, agent_id: str) -> Dict:
        ...
        if (
            subscription.max_usage > 0
            and subscription.usage_count >= subscription.max_usage
        ):
            return {
                "has_access": False,
    ...
    def track_usage(
        self, user_id: str, agent_id: str, usage_type: str = "message"
    ) -> Dict:
        ...
        if subscription and subscription.status == "active":
            ...
            return {
                ...
                "usage_count": subscription.usage_count,
                "remaining_usage": (
                    max(0, subscription.max_usage - subscription.usage_count)
                    if subscription.max_usage > 0
                    else -1
                ),
            }
                ...
                user_subscriptions.append(
                    {
                        ...
                        "usage_count": subscription.usage_count,
                        "days_remaining": max(
                            0, (subscription.end_date - datetime.now()).days
                        ),
                    }
                )
    ...
    def get_payment_analytics(self) -> Dict:
        ...
        total_revenue = sum(
            self.calculate_agent_price(s.agent_id, s.tier)
        ...
        for subscription in self.subscriptions.values():
            # Agent popularity
            agent_popularity[subscription.agent_id] = (
                agent_popularity.get(subscription.agent_id, 0) + 1
            )

            # Payment method stats
            payment_method_stats[subscription.payment_method] = (
                payment_method_stats.get(subscription.payment_method, 0) + 1
            )

            # Tier stats
            tier_stats[subscription.tier.period] = (
                tier_stats.get(subscription.tier.period, 0) + 1
            )
        ...
        return {
            ...
            "conversion_rate": round(
                (active_subscriptions / max(1, total_subscriptions)) * 100, 2
            ),
...
def create_subscription_payment(
    user_id: str, agent_id: str, tier: str, payment_method: str
) -> Dict:
...
def get_user_dashboard_data(user_id: str) -> Dict:
    ...
    return {
        ...
        "total_spent": sum(
            payment_processor.calculate_agent_price(
                s["agent_id"], SubscriptionTier[s["tier"].upper()]
            )
        ...
        "expiring_soon": [
            s
            for s in subscriptions
            if s["days_remaining"] <= 3 and s["status"] == "active"
        ],
    }
