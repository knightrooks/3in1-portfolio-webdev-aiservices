...
class SupportTicket(db.Model):
    ...
    user_id = db.Column(db.String(36), db.ForeignKey('users.id'), nullable=True, index=True)  # Can be null for anonymous
...
class Payment(db.Model):
    ...
    # Product information
    product_type = db.Column(db.String(50), nullable=False)  # 'ai_subscription', 'webdev_service'
...
class WebDevProject(db.Model):
    ...
    project_type = db.Column(db.String(50))  # 'website', 'webapp', 'ecommerce', 'mobile'
    budget = db.Column(db.Decimal(10, 2))
    timeline = db.Column(db.String(50))
    
    # Status
    status = db.Column(db.String(20), default='inquiry')  # inquiry, quoted, approved, in_progress, completed, cancelled
...
class Feedback(db.Model):
    ...
    feedback_type = db.Column(db.String(50), nullable=False)  # bug_report, feature_request, etc.
    message = db.Column(db.Text, nullable=False)
    ...
    # Status
    status = db.Column(db.String(20), default='new')  # new, reviewed, in_progress, resolved, closed
...
def get_user_conversations(user_id, limit=20):
    ...
    conversations = Conversation.query.filter_by(user_id=user_id, is_archived=False)\
