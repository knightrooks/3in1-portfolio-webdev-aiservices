...
import re
from .db import User, db, UserRole
import logging
    ...
    def register_user(email, username, password, first_name=None, last_name=None):
        ...
        try:
            ...
            if not AuthService.validate_username(username):
                raise AuthError("Username must be 3-30 characters, alphanumeric and underscores only")
...
def premium_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if user.role == UserRole.PRO:
            if not user.subscription_expires or user.subscription_expires <= datetime.utcnow():
...
def create_session(user, remember_me=False):
    ...
    try:
        ...
        token = AuthService.generate_jwt_token(user.id, user.role.value, session_duration)
